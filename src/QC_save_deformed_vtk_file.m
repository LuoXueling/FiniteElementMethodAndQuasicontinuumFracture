function QC_save_deformed_vtk_file(datpath,iLoad,nodeCoordRef,elemNodeNo,elemCfg,projectCfg,disp_n1,velo_n1,acce_n1,result,QCbrokenElem)
...
acce=full(acce_n1);
tmpdisp=disp;
% tmpdisp(2:2:end)=tmpdisp(2:2:end)/result.totalDisp;
deformed_nodeCoordRef=nodeCoordRef+tmpdisp;
%Open file
fname=strcat(datpath,'/','QC_deformed_time_',num2str(iLoad),'.vtk');
out=fopen(fname,'w');

%Start writing:
%Header
fprintf(out,'# vtk DataFile Version 2.0\n');
fprintf(out,strcat('deformed_time_',num2str(iLoad),'.vtk\n'));
fprintf(out,'ASCII\n');
...
iconst1=(elemCfg.nElem-length(find(QCbrokenElem)))*(elemCfg.nElemNode+1);

fprintf(out,'CELLS %5d  %5d  \n', elemCfg.nElem-length(find(QCbrokenElem)),iconst1);

for ielem=1:elemCfg.nElem
    if QCbrokenElem(ielem)==1
        continue;
    end
...
fprintf(out,'CELL_TYPES %5d\n',elemCfg.nElem-length(find(QCbrokenElem)));
